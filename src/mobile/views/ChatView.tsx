...
export function ChatView({ sessionId, sessionTitle, onBack, onOpenSession }: { sessionId: string; sessionTitle?: string; onBack: () => void; onOpenSession: (id: string, title?: string) => void }): JSX.Element {
  ...
  useEffect(() => {
    ...
    initialTailPending.current = false
    ...
    void history(sessionId, undefined, 60).then(page => {
      ...
      initialTailPending.current = true
    }).catch(value => {
      if (!alive) return
      historyLoading.current = false
      const buffered = bufferedEvents.current
  ...
  useLayoutEffect(() => {
    ...
    if (!element) return
    const measure = (): void => setComposerHeight(element.getBoundingClientRect().height)
    measure()
  ...
  const loadOlder = async (): Promise<void> => {
    ...
    prependAnchor.current = { height: document.documentElement.scrollHeight, y: window.scrollY }
  ...
  useEffect(() => {
    ...
    const onScroll = (): void => {
      const documentHeight = Math.max(document.documentElement.scrollHeight, document.body?.scrollHeight ?? 0)
      const scrollable = documentHeight > window.innerHeight + 160
      ...
      if (initialPositioned.current && nearTop && hasOlder && !loadingOlder) void loadOlder()
    }
    onScroll()
    firstScroll = false
  ...
  useLayoutEffect(() => {
    ...
    if (anchor) {
      prependAnchor.current = undefined
      const heightDelta = document.documentElement.scrollHeight - anchor.height
      window.scrollTo({ top: anchor.y + heightDelta, behavior: 'auto' })
      return
    }
    if (initialTailPending.current) {
      initialTailPending.current = false
    ...
    if (!loading && stickToBottom.current) scrollToBottom('auto')
  }, [loading, messages])
