...
export interface TunnelState { state: 'stopped' | 'starting' | 'running' | 'failed'; url?: string; error?: string }
export interface PairState { phase: string; deviceCount: number; onlineCount: number; pending: PendingPairing[]; tokenExpiresAt?: number; publicUrl?: string; tunnel: TunnelState }
...
export async function startTunnel(): Promise<{ ok: true; tunnel: TunnelState } | { ok: false; code: string }> {
  const response = await fetch('/harness-remote/pair/tunnel/start', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
  return await response.json() as { ok: true; tunnel: TunnelState } | { ok: false; code: string }
}
